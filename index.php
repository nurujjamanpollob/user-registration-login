...
// Include the login security module
require_once plugin_dir_path(__FILE__) . 'loginsecurity/class-login-security.php';
require_once plugin_dir_path(__FILE__) . 'loginsecurity/config.php';
require_once plugin_dir_path(__FILE__) . 'loginsecurity/settings.php';

// Initialize login security - make sure it's initialized properly
if (!isset($login_security) || !is_object($login_security)) {
...
function register_plugin_assets()
{
    ...
    // Enqueue login security CSS
    wp_register_style('login-security-css', plugin_dir_url(__FILE__) . 'assets/css/login-security.css');
    wp_enqueue_style('login-security-css');
