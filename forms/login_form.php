...
function login_user()
{
    if (isset($_POST['ureglogin_login_username']) && isset($_POST['ureglogin_login_password']) && isset($_POST['_csrf']) && !is_user_logged_in()) {
        ...
        // Check if the user account is locked before attempting authentication
        $locked_accounts = get_option('login_security_locked_accounts', array());
        if (isset($locked_accounts[$username]) && $locked_accounts[$username]['locked']) {
            // Check if lockout period has expired
            $time_window = get_option('login_security_time_window', 60);
            ...
            if (($current_time - $lockout_time) <= ($time_window * 60)) {
                registration_login_errors()->add('account_locked', 'Your account has been locked due to too many failed login attempts. Please reset your password to regain access.');
                return;
            } else {
                // Lockout period expired, unlock the account
                unset($locked_accounts[$username]);
                update_option('login_security_locked_accounts', $locked_accounts);
                
                // Clear any failed attempts for this user
                $failed_attempts = get_option('login_security_failed_attempts', array());
                unset($failed_attempts[$username]);
                update_option('login_security_failed_attempts', $failed_attempts);
            }
        }
        ...
        if (empty($errors)) {
            ...
            if (isset($_POST['ureglogin_login_remember_me'])) { // Fixed variable name from $POST to $_POST
