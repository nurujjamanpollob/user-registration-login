...
function add_new_user()
{
	if (isset($_POST['ureglogin_username']) && isset($_POST['ureglogin_email']) && isset($_POST['_csrf']) && !is_user_logged_in()) {
		...
		// Get configuration options once to avoid multiple calls
		$enable_blacklist = get_option(ENABLE_BLACKLIST_CHECK_OPTION_NAME) === '1';
		$enable_whitelist = get_option(ENABLE_WHITELIST_CHECK_OPTION_NAME) === '1';
		$verify_disposable = get_option(VERIFY_DISPOSABLE_EMAIL_DOMAINS_OPTION_NAME) === '1';

		// Get blacklist verifier only if needed
		...
		// Get whitelist verifier only if needed
		$whitelist_verifier = null;
		if ($enable_whitelist) {
		...
		if ($enable_blacklist && !$enable_whitelist) {
		...
		if ($verify_disposable) {

			$isDisposable = $blacklist_verifier->isDisposableEmail($email);
        ...
        // check if the username is in blocked list
        if ($blacklist_verifier->isUsernameBlocklisted($username)) {
            registration_login_errors()->add('username_blocked', 'This username is not allowed');
        }
